...
class CameraGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # --------------------------------------------
        # Laser control: [LASER BUTTON] [LASER STATUS]
        # --------------------------------------------
        
        layout_laser_control = QtWidgets.QHBoxLayout()
        layout_laser_control.setAlignment(QtCore.Qt.AlignLeft)
        self.main_layout.addLayout(layout_laser_control)
        
        # LASER BUTTON
        self.laser_button = QtWidgets.QPushButton('LASER')
        self.laser_button.setFixedSize(buttonWidth, buttonHeight)
        ...
        # LASER STATUS
        self.laser_status = QtWidgets.QLabel('Status: OFF')
        self.laser_status.setFixedSize(buttonWidth, buttonHeight)
        layout_laser_control.addWidget(self.laser_status)
    ...
    def toggle_laser(self):
        """Code executed when LASER button is toggled"""
        if not self.laser_on:
            self.laser_status.setText('Status: ON')
            ...
            self.u.writeRegister(self.DAC0_REGISTER, 0)
            self.u.writeRegister(self.DAC1_REGISTER, 0)
            self.laser_status.setText('Status: OFF')
    ...
    def receive_frame(self, frame_tuple):
        ...
        laser_status = self.laser_on
        self.frame_cache_input.append([frame_number, timestamp, laser_status, frame])
